...
from bot.database.models import PENDING_PHONE
...
def format_address_label(full_name: str) -> str:
    """Короткая подпись адреса для кнопок выбора."""
    text = full_name.replace("Юрия Якулина", "Юрия Никулина")
    return text.replace("проспект", "пр.")
...
def order_needs_phone(phone: str | None) -> bool:
    if not phone or phone == PENDING_PHONE:
        return True
    return not is_valid_phone(phone)
