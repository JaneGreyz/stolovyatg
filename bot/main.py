...
from bot.handlers.responsible import create_responsible_router
...
from bot.services.monthly_report import monthly_report_loop
from bot.services.topic_cleanup import topic_cleanup_loop
...
async def main() -> None:
    ...
    logger.info("Permanent menu image: %s", settings.permanent_menu_path)

    fsm_path = settings.database_path.with_name(
        f"{settings.database_path.stem}_fsm.sqlite3"
    )
    ...
    dp.include_router(create_responsible_router(settings))
    ...
    cleanup_task = asyncio.create_task(
        topic_cleanup_loop(bot, db, settings, stop_event)
    )
    monthly_report_task = asyncio.create_task(
        monthly_report_loop(bot, db, settings, stop_event)
    )
