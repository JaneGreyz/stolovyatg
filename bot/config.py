...
def _parse_optional_single_id(value: str) -> int | None:
    raw = value.strip()
    if not raw:
        return None
    # На BotHost иногда пишут несколько ID через запятую — берём первый.
...
def load_settings() -> Settings:
    ...
    responsible_staff_id = _parse_optional_single_id(
        os.getenv("RESPONSIBLE_STAFF_ID", "")
    )
