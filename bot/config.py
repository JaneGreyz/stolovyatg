...
class Settings:
    ...
    permanent_menu_file_id: str | None
...
def load_settings() -> Settings:
    permanent_menu_file_id = os.getenv("PERMANENT_MENU_FILE_ID", "").strip() or None
    permanent_menu_path = Path(
        os.getenv("PERMANENT_MENU_PATH", "assets/permanent_menu.png")
    )
    ...
    return Settings(
        ...
        permanent_menu_file_id=permanent_menu_file_id,
...
async def resolve_settings(bot) -> Settings:
    """Resolve @username menu chat to numeric id for filters."""
    import logging
