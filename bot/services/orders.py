...
async def _notify_guest(
    ...
    try:
        ...
        logger.info("Guest %s notified: %s", guest_id, text.split("\n", maxsplit=1)[0])
    except Exception:
        logger.exception(
            "Failed to notify guest %s about order status: %s",
            guest_id,
            text.split("\n", maxsplit=1)[0],
        )
...
async def change_order_status(
    ...
    if new_status == "completed" and not await db.has_review(order.id):
        await _notify_guest(
            bot,
            order.guest_id,
            GUEST_STATUS_NOTIFICATIONS["completed"].format(order_id=order.id),
