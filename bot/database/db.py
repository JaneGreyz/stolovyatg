...
class Database:
    ...
    async def _migrate(self) -> None:
        ...
        await self._migrate_addresses()

    async def _migrate_addresses(self) -> None:
        await self.conn.execute(
            """
            UPDATE addresses
            SET full_name = REPLACE(full_name, 'Ленинградский проспект', 'Ленинградский пр.')
            WHERE full_name LIKE '%Ленинградский проспект%'
            """
        )
        await self.conn.execute(
            """
            UPDATE addresses
            SET full_name = REPLACE(full_name, 'Юрия Якулина', 'Юрия Никулина')
            WHERE full_name LIKE '%Юрия Якулина%'
            """
        )
        for table in ("guest_profiles", "orders"):
            await self.conn.execute(
                f"""
                UPDATE {table}
                SET address = REPLACE(address, 'Ленинградский проспект', 'Ленинградский пр.')
                WHERE address LIKE '%Ленинградский проспект%'
                """
            )
            await self.conn.execute(
                f"""
                UPDATE {table}
                SET address = REPLACE(address, 'Юрия Якулина', 'Юрия Никулина')
                WHERE address LIKE '%Юрия Якулина%'
                """
            )
        await self.conn.commit()
    ...
    async def cancel_pending_orders_for_guest(self, guest_id: int) -> None:
        """Cancel unfinished orders before starting a new checkout."""
        placeholders = ",".join("?" for _ in ACTIVE_STATUSES)
        await self.conn.execute(
            f"""
            UPDATE orders
    ...
    async def update_order_phone(self, order_id: int, phone: str) -> None:
        await self.conn.execute(
            """
            UPDATE orders
            SET phone = ?, updated_at = datetime('now', 'localtime')
            WHERE id = ?
            """,
            (phone, order_id),
        )
