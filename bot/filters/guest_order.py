...
from bot.utils import order_needs_phone
...
class AwaitingPhoneFilter(BaseFilter):
    """Pending checkout order still needs a phone number."""
    ...
    async def __call__(self, message: Message, db: Database) -> bool:
        ...
        return order is not None and order_needs_phone(order.phone)
...
class PendingOrderFilter(BaseFilter):
    ...
    async def __call__(
        ...
        if current in IN_PROGRESS_STATES:
            return False
        order = await db.get_pending_order_for_guest(message.from_user.id)
        return order is not None and not order_needs_phone(order.phone)
