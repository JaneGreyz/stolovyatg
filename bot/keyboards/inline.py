...
def guest_main_keyboard(*, show_admin_switch: bool = False) -> ReplyKeyboardMarkup:
    rows = [
    ...
    if show_admin_switch:
        rows.append([KeyboardButton(text=BUTTON_SWITCH_TO_ADMIN)])
...
def guest_phone_keyboard() -> ReplyKeyboardMarkup:
    """Клавиатура на шаге ввода телефона."""
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text=BUTTON_SEND_PHONE, request_contact=True)],
            [KeyboardButton(text=BUTTON_CANCEL_ORDER)],
        ],
        resize_keyboard=True,
...
def addresses_keyboard(addresses: list[Address]) -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton(text=format_address_label(addr.full_name), callback_data=f"addr:{addr.id}")]
