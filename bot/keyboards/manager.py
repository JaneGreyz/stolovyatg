...
def manager_main_keyboard(*, show_guest_switch: bool = False) -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton(text=BUTTON_MANAGER_REPORT)],
        [
            KeyboardButton(text=BUTTON_MANAGER_TODAY),
            KeyboardButton(text=BUTTON_MANAGER_BACKUP),
        ],
    ]
    if show_guest_switch:
        rows.append([KeyboardButton(text=BUTTON_SWITCH_TO_GUEST)])
