...
def create_admin_router(settings: Settings) -> Router:
    ...
    async def cmd_setmenu(message: Message, db: Database) -> None:
        ...
        parts = (message.text or "").split(maxsplit=1)
        sheet_url = parts[1].strip() if len(parts) > 1 else ""
        ...
        if not message.reply_to_message:
            await message.answer(
                "Два способа:\n\n"
                "1. Постоянное меню — ссылка на Google Sheets:\n"
                "/setmenu https://docs.google.com/spreadsheets/d/.../edit?gid=...\n\n"
                "2. Меню дня из канала — ответьте /setmenu на пересланный пост с #меню"
            )
            return
        ...
        await save_channel_menu_message(db, menu_message_id, menu_chat_id)
        await message.answer("✅ Меню дня из канала сохранено.")
