...
def create_admin_router(settings: Settings) -> Router:
    ...
    async def cmd_monthstats(message: Message, bot: Bot, db: Database) -> None:
        if not message.from_user or not is_admin(message.from_user.id):
            await message.answer(ADMIN_NO_ACCESS)
            return
        ...
        if len(parts) > 1 and parts[1].strip():
            ...
            if len(year_month) != 7 or year_month[4] != "-":
                await message.answer(
                    "Формат: /monthstats 2026-08\n"
                    "Или просто /monthstats — за текущий месяц."
                )
                return
        else:
            year_month = datetime.now(get_tz(settings)).strftime("%Y-%m")
    ...
    async def cmd_cleanup(message: Message, bot: Bot, db: Database) -> None:
        if not message.from_user or not is_admin(message.from_user.id):
            await message.answer(ADMIN_NO_ACCESS)
            return

        result = await run_topic_cleanup(bot, db, settings, force=True)
        if result.get("skipped"):
            await message.answer(ADMIN_CLEANUP_SKIPPED)
            return
        if result.get("backup_failed"):
            await message.answer("Не удалось создать бэкап. Очистка отменена.")
            return

        await message.answer(
            ADMIN_CLEANUP_DONE.format(
    ...
    async def cmd_setmenu(message: Message, db: Database) -> None:
        ...
        if not is_admin(message.from_user.id):
            await message.answer(
                ...
                "Доступ есть только у ответственного сотрудника (RESPONSIBLE_STAFF_ID).",
                parse_mode="HTML",
            )
            return
