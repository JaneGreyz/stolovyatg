...
from bot.utils import extract_phone_from_message, is_valid_phone, normalize_phone
...
async def _complete_order_checkout(
    ...
    for key in ("address", "address_short", "address_id"):
        if key not in data:
            logger.warning(
                "Missing checkout data for guest %s: %s",
...
@router.message(OrderStates.phone, F.text | F.contact)
async def process_phone(
    ...
    await _complete_order_checkout(message, state, db, settings, phone_raw)


@router.message(OrderStates.phone, F.chat.type == "private")
