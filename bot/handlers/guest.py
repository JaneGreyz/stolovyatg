...
from bot.database.models import PENDING_PHONE, Order
...
from bot.utils import format_address_label
...
from bot.filters.user_mode import ResponsibleOnlyFilter
...
async def _format_saved_profile_message(profile) -> str:
    ...
    return SAVED_PROFILE_CONFIRM.format(
        address=format_address_label(profile.address),
...
async def _create_checkout_order(
    ...
    profile = await db.get_guest_profile(guest_id)
    phone = PENDING_PHONE
    if profile and not order_needs_phone(profile.phone):
...
@router.message(F.text == BUTTON_SWITCH_TO_ADMIN, ResponsibleOnlyFilter())
...
async def use_saved_profile(
    ...
    await db.cancel_pending_orders_for_guest(callback.from_user.id)
...
@router.callback_query(
    StateFilter(OrderStates.choosing_address),
    F.data.startswith("addr:"),
)
async def choose_address(
    ...
    await callback.message.edit_text(
        f"📍 {format_address_label(address.full_name)}\n\n{ENTER_ADDRESS_CLARIFICATION}",
...
@router.message(OrderStates.choosing_address, F.text)
async def process_address_text_instead_of_button(message: Message) -> None:
    if not message.text or message.text in MENU_BUTTONS:
        return
    await message.answer("Пожалуйста, выберите адрес из списка кнопок выше.")
...
async def process_clarification(
    ...
    if not message.from_user or not message.text or message.text in MENU_BUTTONS:
        return

    data = await state.get_data()
    for key in ("address", "address_short", "address_id"):
        if key not in data:
            logger.warning(
                "Missing checkout data for guest %s at clarification: %s",
    ...
    guest_id = message.from_user.id
    await db.cancel_pending_orders_for_guest(guest_id)
    order = await _create_checkout_order(message, db, data, clarification)
    ...
    if not order_needs_phone(order.phone):
...
async def _resolve_checkout_order(
    ...
    if order_id:
        order = await db.get_order(order_id)
        if order and order.topic_id is None and order_needs_phone(order.phone):
            return order

    order = await db.get_pending_order_for_guest(message.from_user.id)
    if order and order_needs_phone(order.phone):
        return order
    return None


async def _complete_order_checkout(
    message: Message,
    state: FSMContext,
    db: Database,
    settings: Settings,
    ...
    if not is_valid_phone(phone_raw):
        await message.answer(INVALID_PHONE, reply_markup=guest_phone_keyboard())
        return
    ...
    await db.update_order_phone(order.id, phone)
    ...
    if address_id is None:
        profile = await db.get_guest_profile(guest_id)
        address_id = profile.address_id if profile else address_id
...
@router.message(
    F.chat.type == "private",
    or_f(StateFilter(OrderStates.phone), AwaitingPhoneFilter()),
    F.text | F.contact,
)
async def process_phone(
    ...
    if message.text and message.text in MENU_BUTTONS:
        await message.answer(ENTER_PHONE, reply_markup=guest_phone_keyboard())
        return

    order = await _resolve_checkout_order(message, state, db)
    if not order:
        await state.clear()
        await message.answer(
            ORDER_FLOW_RESTART,
    ...
    phone_raw = extract_phone_from_message(message)
    if not phone_raw:
        await message.answer(INVALID_PHONE, reply_markup=guest_phone_keyboard())
        return

    await _complete_order_checkout(message, state, db, settings, phone_raw, order)
...
async def process_phone_fallback(message: Message) -> None:
    await message.answer(INVALID_PHONE, reply_markup=guest_phone_keyboard())
...
async def _resolve_pending_order(
    ...
    if order_id:
        order = await db.get_order(order_id)
        if order and order.topic_id is None and not order_needs_phone(order.phone):
            return order

    order = await db.get_pending_order_for_guest(message.from_user.id)
    if order and not order_needs_phone(order.phone):
        await state.update_data(order_id=order.id)
        await state.set_state(OrderStates.order_text)
        return order
    return None
