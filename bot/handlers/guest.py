...
from bot.filters.user_mode import AdminCapableFilter, GuestBotModeFilter
...
@router.message(CommandStart(), GuestBotModeFilter())
async def cmd_start(
    message: Message,
    state: FSMContext,
    settings: Settings,
...
@router.message(F.text == BUTTON_START, GuestBotModeFilter())
async def btn_start(
    message: Message,
    state: FSMContext,
    settings: Settings,
