...
from bot.utils import is_valid_phone, normalize_phone, parse_amount
...
async def process_order_text(message: Message, state: FSMContext, settings: Settings) -> None:
    if not message.from_user or not message.text:
        return
    ...
    await state.set_state(OrderStates.order_amount)
    await message.answer(
        ENTER_ORDER_AMOUNT.format(min_amount=settings.min_order_amount),
        reply_markup=cancel_keyboard(),
    )


@router.message(OrderStates.order_amount, F.text)
async def process_order_amount(
    message: Message,
    state: FSMContext,
    db: Database,
    settings: Settings,
    ...
    if amount is None or amount < settings.min_order_amount:
        await message.answer(
            INVALID_ORDER_AMOUNT.format(min_amount=settings.min_order_amount),
            reply_markup=cancel_keyboard(),
        )
        return
    ...
    order = await finalize_order(
        message.bot, db, settings, order, order_text, amount
    )
...
async def forward_guest_message(
    ...
    """Forward guest free-text to staff topic. Must be last handler in this router."""
    if not message.from_user or not message.text:
        return
