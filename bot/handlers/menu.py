...
def _is_daily_menu_post(message: Message, settings: Settings) -> bool:
...
def create_menu_router(settings: Settings) -> Router:
    ...
    async def _capture(message: Message, db: Database) -> None:
        if not _is_daily_menu_post(message, settings):
            return
        await save_channel_menu_message(db, message.message_id, message.chat.id)
        logger.info("Captured daily channel menu in chat %s", message.chat.id)
